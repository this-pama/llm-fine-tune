...
def generate_with_hf_fallback(prompt: str, model_name: Optional[str] = None, max_new_tokens: int = 200, max_length: int = None) -> str:
    ...
    Args:
        prompt: Input prompt
        model_name: HF model name (if None, checks env var then defaults to gpt2)
        max_new_tokens: Maximum new tokens to generate
        max_length: Backward compatibility parameter (ignored if max_new_tokens provided)
    ...
    # Handle backward compatibility: if max_length is provided and max_new_tokens is default, use max_length
    if max_length is not None and max_new_tokens == 200:
